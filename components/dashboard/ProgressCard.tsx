import type { ProgressStatusItem, ProgressSubItem, ProgressStep } from "../../types/models";
...
type ProgressCardProps = {
  item: ProgressStatusItem;
};

export default function ProgressCard({ item }: ProgressCardProps) {
  const stepsFor = (subItem: ProgressSubItem) => {
  ...
  const normalizeSteps = (subItem: ProgressSubItem): ProgressStep[] => {
    ...
    return steps.map((step, index) => {
      ...
      return { number: step as number, status: subItem.status };
    });
  };
